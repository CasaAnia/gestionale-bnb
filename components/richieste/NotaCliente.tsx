...
// proposta e in «Da controllare» in Home. In ROSSO (#8C3B2E, lo stesso degli
// avvisi) da tutte le parti, così non sfugge (Ania, 07/09/2026). Senza nota
// non compare nulla.
...
export default function NotaCliente({ note, className = '', piccola = false }: { note: string | null | undefined; className?: string; piccola?: boolean }) {
  ...
  return (
    <p data-nota-cliente className={`${piccola ? 'text-[12.5px]' : 'text-sm'} leading-snug ${className}`} style={{ color: '#8C3B2E' }}>
      Nota del cliente: <span className="font-semibold">«{testo}»</span>
    </p>
  )
}
