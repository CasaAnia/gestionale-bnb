  ...
  return (
    <div className="fixed inset-0 ed-velo z-[70] overflow-y-auto" onClick={onClose}>
      <div className="bg-cream w-full max-w-lg mx-auto min-h-full p-4" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-2 mb-3">
          {(['contanti', 'bonifico'] as const).map(p => (
            <button key={p} onClick={() => setPagamento(p)}
              className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${pagamento === p ? 'bg-green-mid text-white' : 'text-stone border border-[#C9BFA8]'}`}>
        ...
        <div ref={frameRef} className="rounded-xl overflow-hidden border border-card-border mb-3"
        ...
        <div className="flex gap-2 mb-4">
          <button onClick={condividi} disabled={!!busy}
            className="flex-1 ed-pillola-tenue text-xs disabled:opacity-50">
