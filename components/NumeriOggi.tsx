...
// «Camere occupate» stanotte su quelle attive. Tre riquadri affiancati nello
// stile delle schede (bianco, bordo #C9BFA8, ombra leggera), numero in
// Fraunces ed etichetta in Nunito Sans; toccabili: arrivi e partenze aprono
...
function Riquadro({ href, etichetta, valore, coda }: { href: string; etichetta: string; valore: string; coda?: string }) {
  return (
    <Link href={href} className="bg-white rounded-[10px] border border-[#C9BFA8] shadow-sm px-3 py-3 min-w-0 transition-transform duration-100 active:scale-[0.98]">
      ...
      <p className="text-[26px] leading-none text-green-dark mt-1.5" style={FRAUNCES} data-numero={valore}>
        {valore}{coda && <span className="text-[14px] text-gray-400"> {coda}</span>}
      </p>
    </Link>
  )
}
  ...
  return (
    <section className="mb-4" data-stato={n.stato}>
      <div className="grid grid-cols-3 gap-2">
        ...
        <Riquadro href={`/calendario?giorno=${n.oggi}`} etichetta="Camere occupate" valore={pronto ? String(n.numeri.camereOccupate) : trattino} coda={pronto ? testoOccupate(n.numeri).replace(/^\d+ /, '') : undefined} />
      </div>
