        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-2 mt-2">
            <button onClick={onToggleForm}
              className="rounded-lg py-2 text-xs bg-[#FBF9F4] border border-[#C9BFA8] shadow-sm text-gray-600 transition active:scale-[0.97]">
              {showForm ? '✕ Chiudi' : '＋ Aggiungi'}
            </button>
            <label className="rounded-lg py-2 text-xs bg-[#FBF9F4] border border-[#C9BFA8] shadow-sm text-gray-600 text-center cursor-pointer transition active:scale-[0.97]">
              🖼️ Libreria
          ...
          <div className="grid grid-cols-3 gap-2 mb-2">
            {staged.map((s, i) => (
              <div key={i} className="relative">
                <img src={s.url} alt="anteprima" className="w-full h-24 object-cover rounded-lg border border-[#C9BFA8] shadow-sm" />
            ...
            <div key={r.id} className="relative">
              {receiptUrls[r.id]
                ? <img src={receiptUrls[r.id]} alt="scontrino" onClick={() => window.open(receiptUrls[r.id], '_blank')}
                    className="w-full h-24 object-cover rounded-lg border border-[#C9BFA8] shadow-sm cursor-pointer" />
