  ...
  return (
    <>
      <div className="flex flex-col gap-2.5 mb-3">
        {chat.length === 0 && (
          <div className="self-start max-w-[88%] border border-card-border rounded-2xl rounded-bl-md px-4 py-2.5 text-sm leading-relaxed">
        ...
        {chat.map((b, i) => (
          <div key={i} className={b.io
            ...
            : 'self-start max-w-[88%] border border-card-border rounded-2xl rounded-bl-md px-4 py-2.5 text-sm leading-relaxed'}>
            {b.t}
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-2">
        <input value={domanda} onChange={e => setDomanda(e.target.value)}
          ...
          className="flex-1 ed-campo rounded-xl p-2.5 text-sm" />
