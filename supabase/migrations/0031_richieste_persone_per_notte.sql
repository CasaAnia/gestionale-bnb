-- ============================================================================
-- 0031 — RICHIESTE: persone notte per notte, modifica della richiesta,
--        alternative del caso A (pezzo 9, 02/09/2026)
-- ============================================================================
...
--
-- · richieste.persone_per_notte: jsonb, array di interi lungo quanto le notti
--   (NULL = tutte le notti uguali a `persone`). Caso reale: in 2 la prima
--   notte, poi in 1 (familiari di pazienti Humanitas). Le richieste dal sito
--   arrivano con persone uniche: resta NULL.
-- · richieste.proposte_precedenti: jsonb array; quando una richiesta con
--   proposta inviata viene MODIFICATA in date, persone o camera, la proposta
--   precedente ({testo, soluzione, inviata_at}) finisce qui e lo stato torna
--   in_attesa.
-- · richieste.proposta_alternative: jsonb array di soluzioni; nel caso A con
--   più camere libere il messaggio le elenca tutte e alla conferma Ania sceglie
--   quella accettata dal cliente.
-- · conferma_richiesta: ricontrollo del pool delle 2 brande e letto aggiuntivo
--   NOTTE PER NOTTE con le persone di quella notte; num_guests = massimo.
-- Se le colonne mancano, il gestionale avvisa e non salva persone variabili
-- né modifiche (come per la 0025).

alter table public.richieste add column if not exists persone_per_notte jsonb;
alter table public.richieste add column if not exists proposte_precedenti jsonb not null default '[]'::jsonb;
alter table public.richieste add column if not exists proposta_alternative jsonb;

alter table public.richieste drop constraint if exists richieste_persone_per_notte_array;
alter table public.richieste add constraint richieste_persone_per_notte_array
  check (persone_per_notte is null or pg_catalog.jsonb_typeof(persone_per_notte) = 'array');
alter table public.richieste drop constraint if exists richieste_proposte_precedenti_array;
alter table public.richieste add constraint richieste_proposte_precedenti_array
  check (pg_catalog.jsonb_typeof(proposte_precedenti) = 'array');
...
declare
  ...
  v_partenza   date;
  v_notte      date;
  v_persone    int;      -- persone della RICHIESTA (base)
  v_p          int;      -- persone della notte corrente
  v_p_max      int;      -- massimo nel segmento
  v_letti_seg  int;
  v_letti_presi int;
  v_tel        text;
  v_nome       text;
...
begin
  ...
  -- (0031) persone per notte: se presente dev'essere un array di interi
  -- positivi lungo quanto le notti della richiesta
  v_n_notti := v_r.partenza - v_r.arrivo;
  if v_r.persone_per_notte is not null then
    if pg_catalog.jsonb_typeof(v_r.persone_per_notte) <> 'array'
       or pg_catalog.jsonb_array_length(v_r.persone_per_notte) <> v_n_notti then
      raise exception 'Persone per notte non valide: servono % valori', v_n_notti;
    end if;
    if exists (
      select 1 from pg_catalog.jsonb_array_elements(v_r.persone_per_notte) e
      where pg_catalog.jsonb_typeof(e) <> 'number' or (e::text)::numeric < 1 or (e::text)::numeric <> floor((e::text)::numeric)
    ) then
      raise exception 'Persone per notte non valide: solo interi da 1 in su';
    end if;
  end if;
  ...
  for v_seg in select * from pg_catalog.jsonb_array_elements(v_segmenti) loop
    ...
    while v_notte < v_partenza loop
      -- persone di QUESTA notte (indice rispetto all'arrivo della richiesta)
      v_p := coalesce((v_r.persone_per_notte ->> (v_notte - v_r.arrivo))::int, v_persone);
      -- Letti del pool impegnati questa notte: stessa regola di lib/tariffe
      -- (Amelia parte da 1 posto, le altre da 2; Lena a 3 → 1 letto, a 4 → 2)
      v_letti_seg := case
        when v_nome_cam = 'Lena' then (case when v_p <= 2 then 0 when v_p = 3 then 1 else 2 end)
        when v_p > (case when v_nome_cam = 'Amelia' then 1 else 2 end) then 1
        else 0 end;
      if v_p > (case when v_nome_cam = 'Amelia' then 2 when v_nome_cam = 'Lena' then 4 else 3 end) then
        raise exception 'Camera %: % persone la notte del % % superano la capienza', v_nome_cam, v_p, extract(day from v_notte)::int, v_mesi[extract(month from v_notte)::int];
      end if;
      if exists (
  ...
  --  SOLO nelle notti in cui le persone di quella notte lo richiedono;
  --  num_guests = massimo del segmento)
  for v_seg in select * from pg_catalog.jsonb_array_elements(v_segmenti) order by (value ->> 'arrivo') loop
    ...
    while v_notte < v_partenza loop
      v_p := coalesce((v_r.persone_per_notte ->> (v_notte - v_r.arrivo))::int, v_persone);
      v_p_max := greatest(v_p_max, v_p);
      v_letti_seg := case
        when v_nome_cam = 'Lena' then (case when v_p <= 2 then 0 when v_p = 3 then 1 else 2 end)
        when v_p > (case when v_nome_cam = 'Amelia' then 1 else 2 end) then 1
        else 0 end;
      if v_letti_seg > 0 then
        v_dates := v_dates || pg_catalog.to_jsonb(to_char(v_notte, 'YYYY-MM-DD'));
      end if;
      v_notte := v_notte + 1;
    end loop;
    ...
    insert into public.bookings (
      ...
      v_camera, v_guest, v_arrivo, v_partenza, v_p_max,
      pg_catalog.jsonb_array_length(v_dates) > 0, v_dates,
...
-- I permessi restano quelli della 0027 (grant a authenticated, revoke a public/anon)
...
-- Verifica: devono comparire le tre colonne.
select column_name, data_type, column_default
from information_schema.columns
where table_schema = 'public' and table_name = 'richieste'
  and column_name in ('persone_per_notte', 'proposte_precedenti', 'proposta_alternative')
