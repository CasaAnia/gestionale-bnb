...
1. **Secondo giro**, dopo la verifica indipendente di Codex su `743febd`
   (`VERIFICA-ARRIVO-NAVETTA.md`): cinque bloccanti e cinque correzioni
   circoscritte, tutti chiusi in un giro solo. Base `main` `bb23a39`,
   branch **`arrivo-e-navetta`**, autore unico: Code.
2. **Niente ripiego alla cieca**: si riscrive con le due colonne di sempre
   SOLO se il server dice che manca una colonna della 0058 E non si perde
   niente. Altrimenti non si salva nulla, la bozza resta a schermo e Ania
   legge cosa manca, in parole sue: nessun SQL davanti a lei.
3. **Niente «salvato» senza righe**: la scrittura si fa ridare le righe, si
   rilegge la riga e si confrontano TUTTI i campi (con le due sole colonne
   il cambio di autista passava per riuscito). Freno sincrono al doppio tocco.
4. **La bozza non si perde**: ogni significato ha la sua casella (ora al
   luogo, ora in struttura detta, stima di Ania), cambiare tipo non ne
   svuota nessuna, e sul server va solo la parte che il tipo rende vera.
5. **La testa della scheda legge il modello** (`arrivoTestaDaArrivo`): non
   perde più la fine della fascia, il «circa» e il nome dell'autista.
   Stesso disegno. Anche «Arrivi precedenti» adesso legge il modello.
6. **Home rimessa com'era**: `app/page.tsx` è `main` più dieci righe. La
   riga CHECK-IN, la nota rossa e il «+letto agg.» sono tornati; il riquadro
   nuovo si aggiunge e porta anche lui il letto. 🔴 La duplicazione la decide
   Ania, col confronto prima/dopo nelle schermate.
7. **«Circa» solo per la stima di Ania**: una fascia detta dalla cliente è
   un dato — «fra le 15:00 e le 17:00», non «circa».
8. **0058 rifatta**: undici colonne (un significato ciascuna), la regex non
   accetta più le 29:00, vincoli di coerenza sulle fasce, tutto in una
   transazione. Aggiunti `0058_PIANO_APPLICAZIONE.md` e
   `0058_RIPRISTINO.BOZZA.sql`. **Mai eseguita da nessuna parte.**
9. **Prove**: suite **1705 verdi** (52 in `lib/arrivo.test.ts`), TypeScript,
   build e lint puliti. Dal vero, nell'anteprima senza rete: i cinque
   ingressi con salva/chiudi/riapri/modifica/riapri, i due interruttori
   `senza-0058` e `risposta-vuota`, schermate a 320 e 390 px.
10. **Nessuna pubblicazione, nessuna scrittura sul database vero, nessun
    messaggio a ospiti o autisti.** Non chiedo la pubblicazione: prima
    serve un altro ricontrollo indipendente.

## 🔴 AZIONE PER ANIA

- **Guardare il confronto della Home** (`3-home-prima-390.png` vs
  `3-home-dopo-390.png`) e dire se la riga CHECK-IN resta o va tolta:
  adesso l'arrivo si legge in due posti.
- **Decidere le tre scelte ancora aperte** (nel riscontro): niente riquadri
  grigi, la riga «Navetta da definire» su ogni arrivo, e «senza orario» che
  adesso vuol dire «non se ne sa niente».
- **Decidere se incollare la 0058** su Supabase. Finché non è applicata, un
  arrivo con luogo o autista **non si può salvare**: il gestionale lo dice e
  tiene quello che hai scritto, ma non registra niente a metà.

## Riscontri

- `RISCONTRO-ARRIVO-NAVETTA.md` — le correzioni, le prove e i limiti (Code).
- `VERIFICA-ARRIVO-NAVETTA.md` — la verifica indipendente (Codex).
- `arrivo-navetta-schermate/` — le tre superfici a 390 px, il modulo a 320 px
  e il prima/dopo della Home.

(in `Documents/Codex/2026-09-20/hola-chica-vai-nel-gestionale-e/outputs/`)

## Anteprime sintetiche

- Scheda, inserimento e Arrivi:
  `node scripts/revisioni/anteprima-prenotazioni-finta.mjs` → `localhost:3216`
  (Carmela Sabia porta il caso del riferimento).
  `GET /finto/senza-0058?on=1` le colonne nuove non esistono, in scrittura
  e in lettura; `GET /finto/risposta-vuota?on=1` il PATCH risponde «ok» con
  zero righe e non scrive.
- Home: `node scripts/revisioni/anteprima-home-finta.mjs` → `localhost:3217`
  (Paola Neri è il caso del riferimento, col letto in più).
