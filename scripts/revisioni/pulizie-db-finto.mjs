...
export async function databasePulizieFinto(rooms, bookings, cleanings, { con0059 = false } = {}) {
  const db = new PGlite()
  ...
  for (const p of ['migrations/0018_pulizie_storico.sql', 'migrations/0039_biancheria_recuperata.sql', 'proposte/0045_pulizie_manuali.BOZZA.sql', ...(con0059 ? ['proposte/0059_pulizie_dotazione_tempi.BOZZA.sql'] : [])]) await db.exec(await readFile(new URL(`../../supabase/${p}`, import.meta.url), 'utf8'))
