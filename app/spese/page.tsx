  ...
  return (
    <div className="p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => setShowForm(!showForm)} className="bg-orange-500 text-white rounded-full px-3 py-1.5 text-sm font-semibold">
        ...
        <div className="bg-white rounded-xl p-4 border border-gray-100 mb-4">
          ...
          <div className="grid grid-cols-2 gap-2 mb-2">
            ...
            <input type="number" value={form.amount} onChange={e => setForm({...form, amount: e.target.value})}
              placeholder="Importo €" className="border border-gray-200 rounded-lg p-2 text-sm" />
          </div>
          <input value={form.description} onChange={e => setForm({...form, description: e.target.value})}
            placeholder="Descrizione" className="w-full border border-gray-200 rounded-lg p-2 text-sm mb-3" />
          <button onClick={save} disabled={saving || !form.amount}
            className="w-full bg-orange-500 text-white rounded-xl py-2.5 font-semibold disabled:opacity-50">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <div className="bg-white rounded-xl px-4 py-2 border border-gray-100 text-right">
          <p className="text-xs text-gray-500">Totale mese</p>
          <p className="font-bold text-red-500">€{totale.toFixed(0)}</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-2">
          {filtered.map(e => (
            <div key={e.id} className="bg-white rounded-xl p-3 border border-gray-100 flex items-center justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {e.source === 'email' && <span className="text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full">📧 email</span>}
                </div>
                <p className="text-sm mt-1">{e.description || '—'}</p>
                <p className="text-xs text-gray-400">{e.expense_date}</p>
              </div>
              <div className="flex items-center gap-2">
                <p className="font-bold text-red-500">€{Number(e.amount).toFixed(0)}</p>
