...
function NuovaPrenotazione() {
  ...
  return (
    <div className="p-4">
      ...
      <h1 className="ed-titolo-medio max-lg:hidden mb-4">Nuova prenotazione</h1>
        ...
        <div className="space-y-3">
          <div className="ed-riga py-4">
            ...
            <input
              ...
              className="w-full ed-campo p-3 text-lg mb-3 focus:outline-none focus:border-green-mid"
              autoFocus
            />
              ...
              <div className="flex gap-1.5">
                {([['', 'Da definire'], ['si', 'Sì'], ['no', 'No']] as const).map(([v, label]) => (
                  <button key={v} type="button" onClick={() => setForm({ ...form, shuttle: v })}
                    className={`rounded-full text-sm font-semibold px-4 py-1.5 ${form.shuttle === v ? 'text-white' : 'border border-[#C9BFA8] text-stone'}`}
              ...
              <div className="flex gap-2">
                {([['diretta', 'Diretta'], ['sito_web', '🌐 Sito'], ['whatsapp', 'WhatsApp']] as const).map(([val, label]) => (
                  <button key={val} type="button" onClick={() => setForm({ ...form, source: val })}
                    className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${form.source === val ? 'bg-green-mid text-white' : 'text-stone border border-[#C9BFA8]'}`}>
                    {label}
                  </button>
                ))}
              </div>
