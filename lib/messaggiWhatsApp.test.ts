...
test('«Richiesta orario» per una prenotazione: nome dell\'ospite, testo della scheda, stesso link; null senza telefono', () => {
  ...
  assert.equal(wa.testo, messaggioRichiestaOrario('Anna'))
  // Ogni messaggio comincia con «Gentile [Nome],» (Ania, 21/09/2026)
  assert.ok(wa.testo.startsWith('Gentile Anna,'))
  assert.equal(wa.testo.includes('Anna Rossi'), false)
